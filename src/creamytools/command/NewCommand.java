...
public class NewCommand implements ICommand, IConstants {
    ...
    /**
     * project.propertiesに追加するライブラリ参照設定１
     */
    ...
    /**
     * project.propertiesに追加するライブラリ参照設定２
     */
    ...
    public void exec(String[] args){
        ...
        // ebean.properties ファイルの ebean.search.jars の設定値を更新
        updateEbeanProp(targetDir);
     ...
     * @param targetPath 処理対象のプロジェクトフォルダのパス
     */
    ...
    /**
     * ebean.properties ファイルの ebean.search.jars の設定値を、<userapp>.jer に更新する
     * @param targetPath 処理対象のプロジェクトフォルダのパス
     */
    private void updateEbeanProp(String targetPath){

        // 対象ファイルの存在チェック。プロジェクトディレクトリ配下の prop ディレクトリの配下にある
        File projDir    = new File(targetPath);
        File ebeanProp  = new File(targetPath + SEP + "prop" + SEP + "ebean.properties");
        if(!ebeanProp.exists()){
            System.out.println("ERROR! there is no ebean.properties file on the target project");
            System.exit(ENDCODE_ERROR_EBEANPROP);
        }

       // 全行読み込みながら、必要な行を挿入
        StringBuilder  output    = new StringBuilder();
        try{
            FileReader fr       = new FileReader(ebeanProp);
            BufferedReader br   = new BufferedReader(fr);
            String line;
            while((line = br.readLine()) != null){
                if(line.startsWith("ebean.search.jars")){
                    // 当該設定行をすげ替え
                    output.append("ebean.search.jars=").append(projDir.getName().toLowerCase()).append(".jar").append("\n");
                }
                else{
                    // その他の行はそのまま
                    output.append(line).append("\n");
        ...
        // 変更した内容にて ebean.properties ファイルを上書き
        try {
            FileWriter writer    = new FileWriter(ebeanProp);
            writer.write(output.toString());
            writer.flush();
            writer.close();
        } catch (IOException ex) {
            System.out.println("ERROR! cannot overwride ebean.properties file on the target project");
            System.exit(ENDCODE_ERROR_EBEANPROP);
        }
    }
}
